...
String checkAndGetFullImageURL(String? path, {String? baseUrl}) {
  // baseUrl ??= GSServices.getUser?.baseUrl;
  return switch (isNullEmptyOrFalse(path)) {
    false => startsWithProtocol(path ?? "") ? path! : "$baseUrl$path",
    _ => "",
  };
}

bool startsWithProtocol(String url) {
